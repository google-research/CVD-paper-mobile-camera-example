...
interface SensingEngine {
   ...
   * 1. Unlike some global config settings (like in SDC's QuestionnaireFragment ), each capture may require different capture settings.
   * 2. Given each API call may have different capture settings business logic should be managed by a layer above [CaptureFragment] (not even CaptureViewModel).
   ...
   * This way we also leave other Fragments involved in capturing, like InstructionFragment, out of scope.
   * @param participantId Used to name the capture folder (like Participant_<participantId>).
   * @param captureType type of capture like VIDEO_PPG, IMAGE, etc type of sensor data to be captured
  ...
  /** Lists all ResourceInfo given a captureId. This will return all ResourceInfo for a single capture. */
