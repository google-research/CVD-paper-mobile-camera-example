 ...
 * immediately upon receiving the object in {@code onNext}. Synchronous subscribers should
 * {@link #release} before returning from onNext, while asynchronous subscribers should
 * {@link #release} whenever asynchronous handling is complete. This ensures that Subscribers will
 * not lose access to the underlying resource before they are finished using it.
 *
...
public interface Shared<S extends AutoCloseable> {
  ...
  /**
   * Holds an {@link Shared} object. Closing this Holder is equivalent to calling
   * {@link Shared#release} with the same key.
   *
   * <p>Example usage
   *
   * <pre>{@code
